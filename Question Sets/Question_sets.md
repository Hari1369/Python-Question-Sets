# Python-Question-Sets



# =========================================================================================================#
# =========================================== QUESTIONS ON LIST ============================================#
# =========================================================================================================#
# 1) Create a list of numbers from 1 to 10 and print it.
# 2) Access the 3rd element of a list containing random values.
# 3) Reverse a list without using the built-in reverse function.
# 4) Find the maximum and minimum elements in a list.
# 5) Remove all duplicate values from a list while maintaining the order.
# 6) Sort a list of strings alphabetically.
# 7) Find the index of the first occurrence of an element in a list.
# 8) Count the frequency of each element in a list.
# 9) Join all list elements into a single string.
# 10) Split a list into two sublists: one containing even numbers and the other containing odd numbers.


# =========================================================================================================#
# =========================================== QUESTIONS ON SETS ============================================#
# =========================================================================================================#
# 1) Create a set from a list of numbers, ensuring no duplicates.
# 2) Check if a set is a subset of another set.
# 3) Find the intersection of two sets.
# 4) Find the intersection of two sets.
# 5) Remove an item from a set if it exists.
# 6) Find the difference between two sets.
# 7) Check if two sets are disjoint (i.e., no common elements).
# 8) Convert a list of numbers into a set and find if there are any duplicates in the list.
# 9) Add a new element to a set and then display the set.
# 10) Find the symmetric difference between two sets.


# =========================================================================================================#
# =========================================== QUESTIONS ON TUPLES ==========================================#
# =========================================================================================================#
# 1) Create a tuple with 5 elements and print it.
# 2) Access an element by index from a tuple.
# 3) Concatenate two tuples into one.
# 4) Repeat a tuple multiple times.
# 5) Convert a tuple into a list.
# 6) Find the length of a tuple.
# 7) Count the occurrences of a specific element in a tuple.
# 8) Unpack a tuple into multiple variables
# 9) Create a tuple with mixed data types and access elements using index.
# 10) Sort a tuple (if possible).


# =========================================================================================================#
# ======================================== QUESTIONS ON DICTIONARIES =======================================#
# =========================================================================================================#
# 1) Create a dictionary with keys as strings and values as integers. Add some key-value pairs.
# 2) Access a value from the dictionary using its key.
# 3) Check if a key exists in a dictionary.
# 4) Iterate over all keys in a dictionary.
# 5) Iterate over all values in a dictionary.
# 6) Merge two dictionaries into one.
# 7) Remove a key-value pair from a dictionary.
# 8) Update a value in a dictionary.
# 9) Sort a dictionary by keys and print it.
# 10) Find the key with the maximum value in the dictionary.
